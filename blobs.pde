 ...
 */
 ...
 // hago que identifique el blob y lo mantenga en el tiempo
...
class Blob {
  ...
  void display() {
    ...
    // obtengo el r.x y lo sigo 
    sensor.setTarget( map (r.x-400, xrect-xrect, xrect, r.width, 2000), -10); 
    int manoalto= r.height;
    
    
   //  println ("manoalto" + manoalto);
  // disparo al cerrar mano
     if (manoalto<300){ 
     cursor.disparo(cursor.getX()-10,500); 
     sonidodisparo.play();
